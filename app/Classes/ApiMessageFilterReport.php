...
class ApiMessageFilterReport
{
     ...
     * File report Format
     */
    const DETAILED_MESSAGE_FORMAT = ['MESSAGE_ID', 'DESTINATION', 'MESSAGE_CONTENT', 'ERROR_CODE',"OPERATOR COUNTRY CODE","OPERATOR ID", 'DESCRIPTION_CODE', 'RECEIVE_DATETIME', 'SEND_DATETIME', 'SENDER', 'USER_ID', 'MESSAGE_COUNT', 'OPERATOR', 'SINGLE_PRICE','TOTAL_PRICE'];
    ...
    public function __construct($month = null, $year = null, $userAPI = null, $msgFilter = [])
    {
        ...
        $this->reportDir = public_path('archive/reports/'). $this->year . '/' . $this->month . '/';
        ...
        $this->manifestFile = public_path('archive/reports/') . '.manifest';
    }
    ...
    public function generateReport()
    {
        if ($this->isReportExist()) {
            ...
            foreach ($csvFiles as $file) {
                $reportReader = new \PhpOffice\PhpSpreadsheet\Reader\Csv();
                $reportReader = $reportReader->load($file);

                $worksheet = $reportReader->getActiveSheet();
                $rows = $worksheet->toArray();

                /**
                 * Make sure not insert the header row
                 */
                $counter=1;

                /**
                 * Make sure there is no empty cells in filter message
                 */

                foreach ($rows as $reportIndex => $reportSheet) {
                    $isMatch = false;
                    if ($counter > 1 && !empty($reportSheet)) {
                        $fRow = array_combine(self::DETAILED_MESSAGE_FORMAT, $reportSheet) ?: [];
                        ...
                        foreach ($this->msgFilter as $dept => $value) {
                            foreach ($value as $idx => $content) {
                                ...
                                $this->log->info('ini = '.json_encode($idx));
                                ...
                                if ($matches) {
                                    ...
                                    $isMatch = true;
                                    $this->log->info('is match = '.json_encode($isMatch));
                         ...
                         */
                        if (!$isMatch) {
                            $this->uncategorizedReportWriter->addRow(WriterEntityFactory::createRowFromArray($reportSheet));
    ...
    protected function writeReportFile($arrResult)
    {
        ...
        $startCell = Coordinate::stringFromColumnIndex(1);
        $lastCell = Coordinate::stringFromColumnIndex(5);
        $row = 3;
        $style = $this->getReportStyle();

        $sheet->fromArray(self::FINAL_REPORT_FORMAT)
                ->getParent()->getDefaultStyle()
        ...
        //  save Final Report
        $writer = IOFactory::createWriter($this->reportWriter, 'Xlsx');
    ...
    public function setTrafficValue(&$arrResult, &$fRow, $dept, $rowKey)
    {
        ...
        $arrResult[$dept][$rowKey]['cm'] += $fRow['SINGLE_PRICE'];
        if ($rowKey !== self::DEPT_TOTAL) {
            $arrResult['TOTAL']['ts'] += $fRow['MESSAGE_COUNT'];
            $arrResult['TOTAL']['cm'] += $fRow['SINGLE_PRICE'];
        }
    }
    ...
    {
        $this->reportWriter = new Spreadsheet();
        $this->reportWriter
                ->getProperties()
                ->setCreator("Firstwap")
                ->setTitle($this->reportName);

        $this->uncategorizedReportWriter = WriterEntityFactory::createWriter(Type::XLSX);
        ...
        $this->uncategorizedReportWriter->addRow(WriterEntityFactory::createRowFromArray(self::DETAILED_MESSAGE_FORMAT));
    }
